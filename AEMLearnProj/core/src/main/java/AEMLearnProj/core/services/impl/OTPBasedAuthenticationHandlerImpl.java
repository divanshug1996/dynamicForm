...
import AEMLearnProj.core.schedulers.SimpleScheduledTask.Config;
...
public class OTPBasedAuthenticationHandlerImpl extends DefaultAuthenticationFeedbackHandler implements AuthenticationHandler,AuthenticationFeedbackHandler{

	private final Logger log = LoggerFactory.getLogger(OTPBasedAuthenticationHandlerImpl.class);
	...
	@Activate
	protected void activate(TwoFactorAuthHandler config) {
		pathValue= config.pathValue();
		loginPageValue = config.loginPageValue();
	}
	...
	private boolean check2StepAuthPreference(String userId, Session session1)
		...
		try {
			...
			log.error("Repository exception in boolean method", e);
		}catch(Exception e){
			log.error("Exception in boolean method",e);
		}finally {
